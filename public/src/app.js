...
function _gdsPrepRenderPickingBtns() {
  ...
  entries.forEach(({ pickId, pick }) => {
    ...
    html += `
      <div class="gds-prep-bon-card" onclick="_gdsPrepTogglePickingInclude('${safeName}')" style="cursor:pointer;opacity:${isOn ? '1' : '0.55'};background:${isOn ? 'var(--bg)' : 'var(--bg3)'};">
        ...
        <div class="gds-prep-toggle" data-perm="prep_bon_toggle" style="position:relative;width:26px;height:14px;border-radius:20px;background:${isOn ? 'var(--green)' : 'var(--border)'};flex-shrink:0;pointer-events:none;">
